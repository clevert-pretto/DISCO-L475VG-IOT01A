...
int main (void)
{
    ...
    uart_send_string("\r\n-- 06-watchDog example -- \r\n");
    uart_send_string("--- System Boot / Reset Occurred ---\r\n");

    iwdg_init(); // Start the safety timer
    uart_send_string("Watchdog Armed (2s timeout)\r\n");
    ...
    while(1) {
        if (msTicks >= next_tx) {
            ...
            // FEED THE DOG: If we stop doing this, system resets
            iwdg_feed(); 
            uart_send_string("Dog Fed\r\n");
            
            // SIMULATED HANG: After 10 seconds, enter infinite loop
            if (msTicks > 10000) {
                uart_send_string("SIMULATING SYSTEM HANG...\r\n");
                while(1); // Stop feeding the dog here
            }
        }
    }
}