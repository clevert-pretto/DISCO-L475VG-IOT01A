...
/* cppcheck-suppress [constParameterPointer, misra-c2012-8.4] */
    void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
    {
        if (huart != NULL) 
        {
            if (huart->Instance == USART1 && FreeRTOS_Cpp::appLogger::instance != nullptr)
            {
                BaseType_t xHigherPriorityTaskWoken = pdFALSE;
                
                /* Send the received byte to the Command Task */
                (void)xQueueSendFromISR(FreeRTOS_Cpp::appLogger::instance->_commandQueue, 
                                        const_cast<uint8_t*>(&FreeRTOS_Cpp::appLogger::instance->_rxChar),
                            &xHigherPriorityTaskWoken);

                /* Re-enable the interrupt for the next character */
                (void)HAL_UART_Receive_IT(huart, const_cast<uint8_t*>(&FreeRTOS_Cpp::appLogger::instance->_rxChar), 1u);
...
namespace FreeRTOS_Cpp
{
    ...
    void appLogger::sendCommandResponse(const char *pMsg)
    {
        ...
        {
            (void)HAL_UART_Transmit(_huart, reinterpret_cast<const uint8_t*>(pMsg), (uint16_t)strlen(pMsg), 100U);
            (void)xSemaphoreGive(_uartMutex);
        }
    }
}